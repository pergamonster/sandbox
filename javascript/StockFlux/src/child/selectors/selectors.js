...
export const appSelector = createCurrentWindowStateSelector((state) => {
    const { selection, windowState } = state;
    const { name, code } = selection;
    return { name, code, windowState };
});

export const favouritesSelector = createCurrentWindowStateSelector((state) => {
    const { favourites, selection, windowState } = state;
    return {
        favourites, selection, windowState, isStarting: false, hasErrors: false
    };
});
export const initialOpenSelector = createCurrentWindowStateSelector((state) => {
    const { initialOpen } = state;
    return initialOpen;
});

export const searchSelector = createCurrentWindowStateSelector((state) => {
    const { favourites, selection } = state;
    const {
        isSearching, hasErrors, results, term
    } = state.search;
    return {
        favourites, isSearching, hasErrors, results, term, selection
    };
});
...
export const sidebarSelector = createSelector(
    ...
    (closedWindows, currentWindowState) => {
        ...
        const {
            sidebar, selection, favourites, windowState
        } = currentWindowState;
        return {
            sidebar, selection, favourites, windowState, closedWindowsCount
        };
    }
);

export const toolbarSelector = createCurrentWindowStateSelector((state) => {
