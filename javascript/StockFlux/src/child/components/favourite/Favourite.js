...
class Favourite extends Component {
    ...
    onDragEnd(e) {
        if (e.dataTransfer.dropEffect === 'none') {
            ...
            stockData = stockData || {};
            this.props.bindings.onDropOutside(this.props.stockCode, stockData.name, {
                screenX, screenY, clientX, offsetY
            });
        }
    ...
    render() {
        const {
            stockCode, selected, bindings, isUnfavouriting, dragOver, dragOverBottom
        } = this.props;

        let { stockData } = this.state || {};
        const {
            isHovered, starTop, isDragging, chartData
        } = this.state || {};
        stockData = stockData || {};
